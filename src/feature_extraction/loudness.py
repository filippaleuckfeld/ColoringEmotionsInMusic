...
def analyze_loudness(data: np.ndarray, sr: int = 44100, block_size: float = 0.4, plot_graph: bool = False) -> Dict:
    ...
    try:
        ...
        if plot_graph:
            plot_loudness_over_time(segment_loudness, block_size, len(data) / sr)
...
def plot_loudness_over_time(segment_loudness: np.ndarray, block_size: float, total_duration: float):
    try:
        time_axis = np.arange(0, total_duration, block_size)
